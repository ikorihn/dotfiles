...
local data_dir = wezterm.home_dir .. "/.local/share/wezterm/session-manager/"
...
local function display_notification(window, message)
  wezterm.log_info("WezTerm Session Manager >", message)
  ...
  for _, tab_data in ipairs(workspace_data.tabs) do
    ...
    for j, pane_data in ipairs(tab_data.panes) do
      ...
      if j == 1 then
        ...
        if pane_data.left == tab_data.panes[j - 1].left then direction = "Bottom" end
...
local function load_from_json_file(file_path)
  ...
  if not data then wezterm.log_info("Failed to parse JSON data from file: " .. file_path) end
  return data
end
...
function session_manager.save_state(window)
  ...
  local file_path = data_dir .. data.name .. ".json"
